...
export const composeDigestBody = (data: DailyDigestData): string => {
	...
	lines.push(
		`Processed: ${data.processedCount} / ${data.dailyLimit} | Action needed: ${data.actionRequiredCount} | Auto-recycling: ${data.autoRecyclingCount} | Space freed by auto-recycler: ${spaceFreedStr}`
	);
	lines.push('');
...
export const composeDigestHtml = (data: DailyDigestData): string => {
	...
	<div style="font-size:13px;color:#6b7280;margin-bottom:16px;">Processed: ${data.processedCount} / ${data.dailyLimit} &nbsp;·&nbsp; Action needed: ${data.actionRequiredCount} &nbsp;·&nbsp; Auto-recycling: ${data.autoRecyclingCount} &nbsp;·&nbsp; Space freed by auto-recycler: ${spaceFreedStr}</div>
	${storageHtml}
	${overflowHtml}
	${sections.join('')}
	${recyclingFooterHtml}
