...
const renderItemHtml = (item: PendingItem): string => {
	const subjectDisplay =
		item.subject && item.subject.trim() ? item.subject.trim() : '(no subject)';

	return `<div id="pending-${escapeHtml(item.threadId)}" style="padding:10px 0;border-top:1px solid rgba(0,0,0,0.06);">
